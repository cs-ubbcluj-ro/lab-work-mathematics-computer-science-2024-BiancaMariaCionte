...
extern int yylineno;  // Declared in the Lex file
extern char *yytext;  // Declared in the Lex file

void yyerror(const char *s) {
    // Print the error message and the line number
    fprintf(stderr, "Error at line %d: %s\n", yylineno, s);

    // Print the current token if available (yytext holds the current token's string)
    fprintf(stderr, "Current token: %s\n", yytext);

    exit(1);  // Exit on error (you can adjust this depending on your needs)
}
